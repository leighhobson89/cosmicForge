...
### Timer, Event, and Automation System (`timerManager.js`)

This section documents the timer and automation system, which underpins all periodic updates, automation, and event-driven logic in Cosmic Forge. It explains the architecture in `timerManager.js` and how timers are used by `game.js` and `ui.js` to drive gameplay and UI updates.

**Overview:**
- The timer system is centralized in `timerManager.js`, which exports a singleton `timerManager`.
- The system provides robust management for recurring and one-off timed actions, supporting pausing, resuming, and stopping timers.
- Timers are used for resource generation, automation, UI refreshes, event triggers, and more.

**Core Classes:**
- `TimerManager`: Manages all timers in the game using a Map keyed by unique timer names. Provides methods to add, remove, stop, and retrieve timers.
  - `addTimer(key, duration, onExpire)`: Adds a new timer with a unique key, interval duration (ms), and callback function.
  - `removeTimer(key)`: Stops and removes a timer by key.
  - `stopAllTimers()`: Stops all active timers.
  - `getTimer(key)`: Retrieves a timer instance by key.
- `Timer`: Represents an individual timer. Handles interval logic, tracks elapsed time, and provides methods to start, stop, pause, and resume.
  - `start()`: Begins the timer, calling the callback at each interval.
  - `stop()`: Stops the timer.
  - `pause()`: Pauses the timer without removing it.
  - `resume()`: Resumes a paused timer.

**How Timers Are Used in the Game:**
- **Resource Generation:** Timers periodically invoke resource production and consumption logic, ensuring resources accumulate or are consumed automatically over time.
- **Automation:** Auto-buyers, auto-sell, and fusion processes are triggered by timers, enabling hands-off progression and batch actions.
- **UI Refresh:** Timers ensure UI elements (resource panels, notifications, tooltips) are updated at regular intervals to reflect the latest game state.
- **Event Triggers:** Special events (e.g., achievements, star system changes, weather events) are managed by timers for scheduled or recurring effects.
- **Game Loop:** The main game loop in `game.js` may use timers for periodic updates, state checks, and deferred actions.
- **Autosave:** The autosave feature is implemented using a timer to periodically serialize and store game state.

**Integration with `game.js` and `ui.js`:**
- `game.js` sets up and manages gameplay-related timers for resource, automation, and event logic.
- `ui.js` uses timers to periodically refresh UI components, update tooltips, and manage notification lifecycles.
- Both modules interact with the `timerManager` via its API, ensuring all timed actions are coordinated and lifecycle-managed.

**Extending and Adding Timers:**
- To add a new timed feature, use `timerManager.addTimer('uniqueKey', durationMs, callbackFn)`.
- Remove timers with `timerManager.removeTimer('uniqueKey')` when no longer needed (e.g., on rebirth, tab switch, or feature disable).
- Pause/resume timers as needed for gameplay events, UI focus/blur, or performance.
- All timers should use unique keys to avoid conflicts and ensure safe management.

**Cross-References:**
- **Resource Data Model:** Timers frequently update resource quantities and trigger automation logic.
- **UI System:** Timers drive UI refreshes, notifications, and dynamic content updates.
- **Save/Load System:** Timers may be paused or reset during save/load and rebirth events to ensure consistency.

**Summary:**
- The timer/event/automation system is a backbone of Cosmic Forge, enabling real-time progression, automation, and dynamic UI updates. Its centralized, extensible design makes it easy to add new timed features and ensures robust, maintainable gameplay and interface logic.
...
- **Automation and Timers:**
  - The `timerManager` manages all recurring and delayed actions, including resource production, research, rocket travel, and automation cycles.
