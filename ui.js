...
import {
    ...
    modalEventEndlessSummerHeader,
    modalEventEndlessSummerText,
    modalEventEndlessSummerEndedHeader,
    modalEventEndlessSummerEndedText,
...
import { drawTab9Content } from './drawTab9Content.js';
    ...
    if (type === 'intro') {      
        ...
        switch (tab) {
            ...
            case 'tab8':
                ...
                break;
            case 'tab9':
                optionDescriptionElement = optionDescriptionElements[8];
                optionDescriptionElement.innerHTML = optionDescription;
                optionDescriptionElement.style.border = `1px dashed var(--container-border-color)`;
...
const attentionRules = [
    ...
    selector: '#tab9',
    condition: () => {
        const container = document.getElementById('tab9ContainerGroup');
        return container && container.querySelector('.attention-indicator') !== null;
    }
    }
  ];
...
export function createHtmlTableStatistics(id, classList = [], mainHeadings, subHeadings, subBodys, mainHeaderClasses = [], subHeaderClasses = [], subBodyClasses = []) {
    ...
    for (let i = 0; i < mainHeadings.length; i++) {
        ...
        const dualColumnPerRow = ['resources', 'compounds', 'research', 'energy', 'spacemining', 'interstellar', 'galactic casino'].includes(String(mainHeadings[i]).toLowerCase());
...
export function showTabsUponUnlock() {
    ...
    if (!cosmicRipEnabled) {
        ...
        if (tab8) tab8.remove();
        if (tab8Container) tab8Container.remove();
    }
...
function initializeTabEventListeners() {
    ...
    const ua = (typeof window !== 'undefined' && window.navigator?.userAgent) ? window.navigator.userAgent.toLowerCase() : '';
    const isElectron = ua.includes('electron') || (typeof window !== 'undefined' && window.process?.versions?.electron);
    const exitGameOption = document.getElementById('tab9ExitGameOption');
    if (exitGameOption) {
    ...
    document.querySelectorAll('[class*="tab9"][class*="option15"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'exit game');
            setCurrentOptionPane('exit game');
            updateContent('Exit Game', 'tab9', 'content');
            setFirstAccessArray('exit game');
        });
    });


    document.querySelectorAll('[class*="tab9"][class*="option1"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'visual');
            setCurrentOptionPane('visual');
            updateContent('Visual', 'tab9', 'content');
            setFirstAccessArray('visual');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option2"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'saving / loading');
            setCurrentOptionPane('saving / loading');
            updateContent('Saving / Loading', 'tab9', 'content');
            setFirstAccessArray('saving / loading');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option3"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'game options');
            setCurrentOptionPane('game options');
            updateContent('Game Options', 'tab9', 'content');
            setFirstAccessArray('game options');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option4"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'get started');
            setCurrentOptionPane('get started');
            updateContent('Get Started', 'tab9', 'content');
            setFirstAccessArray('get started');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option5"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'concepts - early');
            setCurrentOptionPane('concepts - early');
            updateContent('Concepts - Early', 'tab9', 'content');
            setFirstAccessArray('concepts - early');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option6"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'concepts - mid');
            setCurrentOptionPane('concepts - mid');
            updateContent('Concepts - Mid', 'tab9', 'content');
            setFirstAccessArray('concepts - mid');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option7"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'concepts - late');
            setCurrentOptionPane('concepts - late');
            updateContent('Concepts - Late', 'tab9', 'content');
            setFirstAccessArray('concepts - late');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option13"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'concepts - end goal');
            setCurrentOptionPane('concepts - end goal');
            updateContent('Concepts - End Goal', 'tab9', 'content');
            setFirstAccessArray('concepts - end goal');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option8"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'statistics');
            setCurrentOptionPane('statistics');
            updateContent('Statistics', 'tab9', 'content');
            setFirstAccessArray('statistics');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option9"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'contact');
            setCurrentOptionPane('contact');
            updateContent('Contact', 'tab9', 'content');
            setFirstAccessArray('contact');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option10"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'achievements');
            setCurrentOptionPane('achievements');
            refreshAchievementTooltipDescriptions();
            updateContent('Achievements', 'tab9', 'content');
            setFirstAccessArray('achievements');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option14"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'events');
            setCurrentOptionPane('events');
            updateContent('Events', 'tab9', 'content');
            setFirstAccessArray('events');
            removeAttentionIndicator(this);
            removeTabAttentionIfNoIndicators('tab9');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option11"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'philosophies');
            setCurrentOptionPane('philosophies');
            updateContent('Philosophies', 'tab9', 'content');
            setFirstAccessArray('philosophies');
        });
    });

    document.querySelectorAll('[class*="tab9"][class*="option12"]').forEach(function(element) {
        element.addEventListener('click', function() {
            selectRowCss(this);
            setLastScreenOpenRegister('tab9', 'story');
            setCurrentOptionPane('story');
            updateContent('Story', 'tab9', 'content');
...
function addWackyEffectsEventListeners() {
    ...
    prizeTickerSpan.addEventListener('click', () => {
        ...
        if (effectItem === 'feedback') {
            setActivatedWackyNewsEffectsArray(effectItem, getDemoBuild() ? 'good (demo)' : 'good');
    ...
    if (prizeTickerSpan2 && prizeTickerSpan2.hasAttribute('data-effect-item')) {
        prizeTickerSpan2.addEventListener('click', () => {
            ...
            if (effectItem === 'feedback') {
                setActivatedWackyNewsEffectsArray(effectItem, getDemoBuild() ? 'bad (demo)' : 'bad');
                if (getFeedbackCanBeRequested()) {
...
setRandomEventUiHandlers({
    ...
    showEventModal: (eventId, replacements) => {
        const headerMap = {
            ...
            endlessSummer: modalEventEndlessSummerHeader,
            endlessSummerEnded: modalEventEndlessSummerEndedHeader,
        ...
        const textMap = {
            ...
            endlessSummer: modalEventEndlessSummerText,
            endlessSummerEnded: modalEventEndlessSummerEndedText,
